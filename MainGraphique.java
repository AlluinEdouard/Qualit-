...
class MainGraphique {
	...
	public static void dessinerPlateau(Fenetre f, Plateau p, char couleurQuiJoue) {
		...
		// Ajout des Favaliers
		ArrayList<Piece> listeF = p.getPiecesFavalier(); // Cette méthode doit être ajoutée dans la classe Plateau
		for (Piece pi : listeF) {
			String nomImage = "./images/" + pi.getNomLong() + ".png";
			f.ajouter(new Texture(nomImage,
					new Point(pi.getPosition().getX() * MainGraphique.tailleCase,
							pi.getPosition().getY() * MainGraphique.tailleCase),
					MainGraphique.tailleCase, MainGraphique.tailleCase));
		}
